import { GemeenteData } from "./gemeente-data"

export const rijnmondData: Record<string, GemeenteData> = {
  "goeree-overflakkee": {
    slug: "goeree-overflakkee",
    naam: "Goeree-Overflakkee",
    regio: "Rijnmond",
    title: "Woningontruiming Goeree-Overflakkee - Laagste Prijs Garantie",
    description:
      "Professionele woningontruiming op Goeree-Overflakkee. Vindt u goedkoper? Wij duiken onder die prijs! Alle kernen bereikbaar.",
    wijken: ["Middelharnis", "Oude-Tonge", "Dirksland", "Sommelsdijk", "Stellendam", "Ouddorp"],
    postcodes: "3240-3259",
    introText:
      "Goeree-Overflakkee is een eiland met vele kernen. Wij bieden professionele woningontruiming in alle dorpen en steden op het eiland tegen de laagste prijs, gegarandeerd.",
    waaromKiezen: [
      "Bekend met het eilandkarakter van Goeree-Overflakkee",
      "Actief in alle kernen",
      "Flexibele planning",
      "Laagste prijs garantie",
      "Ervaring met vakantiewoningen en permanente bewoning",
    ],
    situaties: [
      {
        title: "Vakantiehuis ontruimen",
        description:
          "Veel vakantiewoningen op het eiland. Wij helpen bij ontruiming en bezemschoon opleveren.",
      },
      {
        title: "Ontruiming na overlijden",
        description: "Met respect en zorg helpen wij bij het ontruimen van een woning na overlijden.",
      },
      {
        title: "Seniorenverhuizing",
        description: "Bij verhuizing naar het vasteland begeleiden wij u volledig.",
      },
      {
        title: "Woningontruiming alle kernen",
        description: "Van Middelharnis tot Ouddorp, wij werken overal op Goeree-Overflakkee.",
      },
    ],
    faq: [
      {
        question: "Werken jullie in alle kernen?",
        answer:
          "Ja, wij werken overal op Goeree-Overflakkee: Middelharnis, Sommelsdijk, Dirksland, Oude-Tonge, Ouddorp, Stellendam en alle andere kernen.",
      },
      {
        question: "Hoe zit het met de bereikbaarheid?",
        answer: "Wij plannen onze routes efficiënt en zorgen voor tijdige aankomst op het eiland.",
      },
      {
        question: "Ontruimen jullie ook vakantiewoningen?",
        answer: "Ja, wij hebben ervaring met zowel vakantiewoningen als permanent bewoonde huizen.",
      },
    ],
  },
  "krimpen-aan-den-ijssel": {
    slug: "krimpen-aan-den-ijssel",
    naam: "Krimpen aan den IJssel",
    regio: "Rijnmond",
    title: "Woningontruiming Krimpen aan den IJssel - Laagste Prijs Garantie",
    description:
      "Professionele woningontruiming in Krimpen aan den IJssel. Vindt u goedkoper? Wij duiken onder die prijs! Alle wijken.",
    wijken: ["Krimpen Centrum", "Stormpolder", "Lekkerkerk"],
    postcodes: "2920-2925",
    introText:
      "Krimpen aan den IJssel is een gemeente aan de rivier met een mix van woningtypes. Wij bieden professionele woningontruiming tegen de laagste prijs, gegarandeerd.",
    waaromKiezen: [
      "Kennis van Krimpen en Lekkerkerk",
      "Snelle bereikbaarheid vanuit Rotterdam",
      "Ervaring met alle woningtypes",
      "Laagste prijs garantie",
      "Persoonlijke service",
    ],
    situaties: [
      {
        title: "Woningontruiming Lekkerkerk",
        description: "Ook in Lekkerkerk bieden wij volledige ontruimingsdiensten aan.",
      },
      {
        title: "Huurwoning bezemschoon opleveren",
        description: "Wij zorgen dat uw huurwoning volledig voldoet aan de oplevervoorwaarden.",
      },
      {
        title: "Ontruiming na overlijden",
        description: "Met respect helpen wij bij het ontruimen na verlies van een dierbare.",
      },
      {
        title: "Spoedontruiming",
        description: "Bij spoed kunnen we vaak binnen 24 uur starten.",
      },
    ],
    faq: [
      {
        question: "Werken jullie ook in Lekkerkerk?",
        answer: "Ja, wij werken in heel Krimpen aan den IJssel inclusief Lekkerkerk.",
      },
      {
        question: "Hoe snel kunnen jullie starten?",
        answer: "Meestal binnen 1-3 werkdagen, bij spoed binnen 24 uur.",
      },
      {
        question: "Wat is de laagste prijs garantie?",
        answer: "Vindt u het elders goedkoper? Wij duiken onder die prijs!",
      },
    ],
  },
  lansingerland: {
    slug: "lansingerland",
    naam: "Lansingerland",
    regio: "Rijnmond",
    title: "Woningontruiming Lansingerland - Laagste Prijs Garantie",
    description:
      "Professionele woningontruiming in Lansingerland. Vindt u goedkoper? Wij duiken onder die prijs! Berkel en Rodenrijs, Bleiswijk en Bergschenhoek.",
    wijken: ["Berkel en Rodenrijs", "Bleiswijk", "Bergschenhoek"],
    postcodes: "2650-2665",
    introText:
      "Lansingerland bestaat uit drie kernen: Berkel en Rodenrijs, Bleiswijk en Bergschenhoek. Wij bieden professionele woningontruiming in alle kernen tegen de laagste prijs.",
    waaromKiezen: [
      "Actief in alle drie de kernen van Lansingerland",
      "Kennis van lokale situatie",
      "Snel beschikbaar",
      "Laagste prijs garantie",
      "Ervaring met diverse woningtypes",
    ],
    situaties: [
      {
        title: "Woningontruiming Berkel en Rodenrijs",
        description: "Professionele ontruiming in de grootste kern van Lansingerland.",
      },
      {
        title: "Ontruiming Bleiswijk en Bergschenhoek",
        description: "Ook in Bleiswijk en Bergschenhoek zijn wij uw partner.",
      },
      {
        title: "Seniorenverhuizing",
        description: "Begeleiding bij verhuizing naar een zorginstelling.",
      },
      {
        title: "Bezemschoon opleveren",
        description: "Volgens alle voorwaarden van de verhuurder.",
      },
    ],
    faq: [
      {
        question: "Werken jullie in alle kernen?",
        answer: "Ja, wij werken in Berkel en Rodenrijs, Bleiswijk én Bergschenhoek.",
      },
      {
        question: "Hoe snel kunnen jullie er zijn?",
        answer: "Meestal binnen 1-3 werkdagen, bij spoed sneller.",
      },
      {
        question: "Ontruimen jullie ook bedrijfspanden?",
        answer: "Ja, naast woningen doen we ook bedrijfsontruimingen.",
      },
    ],
  },
  maassluis: {
    slug: "maassluis",
    naam: "Maassluis",
    regio: "Rijnmond",
    title: "Woningontruiming Maassluis - Laagste Prijs Garantie",
    description:
      "Professionele woningontruiming in Maassluis. Vindt u goedkoper? Wij duiken onder die prijs! Haven stad met persoonlijke service.",
    wijken: ["Maassluis Centrum", "Steendijkpolder", "Koningshoek"],
    postcodes: "3140-3149",
    introText:
      "Maassluis is een havenstad met karakter. Wij bieden professionele woningontruiming in heel Maassluis tegen de laagste prijs, gegarandeerd.",
    waaromKiezen: [
      "Kennis van Maassluis en haven omgeving",
      "Ervaring met oudere en nieuwere wijken",
      "Persoonlijke aanpak in kleinere stad",
      "Laagste prijs garantie",
      "Snel beschikbaar",
    ],
    situaties: [
      {
        title: "Ontruiming historische panden",
        description: "Maassluis kent karakteristieke oudere woningen. Wij werken met zorg en aandacht.",
      },
      {
        title: "Huurwoning opleveren",
        description: "Bezemschoon opleveren volgens alle voorwaarden.",
      },
      {
        title: "Ontruiming na overlijden",
        description: "Met respect en discretie.",
      },
      {
        title: "Spoedontruiming Maassluis",
        description: "Binnen 24-48 uur beschikbaar bij spoed.",
      },
    ],
    faq: [
      {
        question: "Hebben jullie ervaring met oudere panden?",
        answer: "Ja, wij hebben veel ervaring met historische en karakteristieke woningen in Maassluis.",
      },
      {
        question: "Hoe snel kunnen jullie starten?",
        answer: "Meestal binnen 1-3 werkdagen, bij spoed binnen 24 uur.",
      },
      {
        question: "Wat is de laagste prijs garantie?",
        answer: "Vindt u het elders goedkoper? Wij duiken onder die prijs!",
      },
    ],
  },
  nissewaard: {
    slug: "nissewaard",
    naam: "Nissewaard",
    regio: "Rijnmond",
    title: "Woningontruiming Nissewaard - Laagste Prijs Garantie",
    description:
      "Professionele woningontruiming in Nissewaard. Vindt u goedkoper? Wij duiken onder die prijs! Spijkenisse en alle dorpen.",
    wijken: ["Spijkenisse", "Abbenbroek", "Geervliet", "Heenvliet", "Hekelingen", "Simonshaven", "Zuidland", "Oudenhoorn"],
    postcodes: "3200-3209",
    introText:
      "Nissewaard bestaat uit de stad Spijkenisse en zeven dorpen: Abbenbroek, Geervliet, Heenvliet, Hekelingen, Simonshaven, Zuidland en Oudenhoorn. Wij bieden professionele woningontruiming in alle kernen tegen de laagste prijs, gegarandeerd.",
    waaromKiezen: [
      "Actief in heel Nissewaard - stad én dorpen",
      "Kennis van alle 8 kernen",
      "Ervaring met verschillende woningtypes",
      "Laagste prijs garantie",
      "Flexibele planning voor zowel stad als dorpen",
    ],
    situaties: [
      {
        title: "Woningontruiming Spijkenisse",
        description: "Professionele ontruiming in Spijkenisse, alle wijken en hoogbouw.",
      },
      {
        title: "Ontruiming dorpen Nissewaard",
        description: "Ook in Abbenbroek, Geervliet, Heenvliet, Hekelingen, Simonshaven, Zuidland en Oudenhoorn zijn wij actief.",
      },
      {
        title: "Hoogbouw appartement ontruimen",
        description: "Ervaring met hoogbouw in Spijkenisse.",
      },
      {
        title: "Seniorenverhuizing",
        description: "Volledige begeleiding bij verhuizing naar zorginstelling.",
      },
    ],
    faq: [
      {
        question: "Werken jullie in alle kernen van Nissewaard?",
        answer: "Ja, wij werken in heel Nissewaard: Spijkenisse en alle 7 dorpen (Abbenbroek, Geervliet, Heenvliet, Hekelingen, Simonshaven, Zuidland en Oudenhoorn).",
      },
      {
        question: "Hoe snel kunnen jullie starten?",
        answer: "Meestal binnen 1-3 werkdagen, bij spoed binnen 24 uur.",
      },
      {
        question: "Hebben jullie ervaring met hoogbouw?",
        answer: "Ja, wij hebben veel ervaring met hoogbouw appartementen in Spijkenisse.",
      },
    ],
  },
  ridderkerk: {
    slug: "ridderkerk",
    naam: "Ridderkerk",
    regio: "Rijnmond",
    title: "Woningontruiming Ridderkerk - Laagste Prijs Garantie",
    description:
      "Professionele woningontruiming in Ridderkerk. Vindt u goedkoper? Wij duiken onder die prijs! Alle wijken bereikbaar.",
    wijken: ["Ridderkerk Centrum", "Bolnes", "Rijsoord", "Slikkerveer"],
    postcodes: "2980-2989",
    introText:
      "Ridderkerk is een gemeente met diverse wijken en kernen. Wij bieden professionele woningontruiming in heel Ridderkerk tegen de laagste prijs, gegarandeerd.",
    waaromKiezen: [
      "Kennis van alle wijken in Ridderkerk",
      "Ervaring met verschillende woningtypes",
      "Snel beschikbaar",
      "Laagste prijs garantie",
      "Ook actief in Bolnes, Rijsoord en Slikkerveer",
    ],
    situaties: [
      {
        title: "Woningontruiming alle kernen",
        description: "Van Ridderkerk centrum tot Bolnes, Rijsoord en Slikkerveer.",
      },
      {
        title: "Huurwoning opleveren",
        description: "Bezemschoon volgens voorwaarden verhuurder.",
      },
      {
        title: "Ontruiming na overlijden",
        description: "Met respect en zorg tijdens moeilijke periode.",
      },
      {
        title: "Spoedontruiming",
        description: "Bij spoed kunnen we snel starten.",
      },
    ],
    faq: [
      {
        question: "Werken jullie in alle kernen?",
        answer: "Ja, wij werken in heel Ridderkerk inclusief Bolnes, Rijsoord en Slikkerveer.",
      },
      {
        question: "Hoe snel kunnen jullie er zijn?",
        answer: "Meestal binnen 1-3 werkdagen, bij spoed sneller.",
      },
      {
        question: "Ontruimen jullie ook bedrijfspanden?",
        answer: "Ja, wij doen ook bedrijfsontruimingen in Ridderkerk.",
      },
    ],
  },
  rotterdam: {
    slug: "rotterdam",
    naam: "Rotterdam",
    regio: "Rijnmond",
    title: "Woningontruiming Rotterdam - Laagste Prijs Garantie",
    description:
      "Professionele woningontruiming in Rotterdam. Vindt u goedkoper? Wij duiken onder die prijs! Alle wijken, 24/7 bereikbaar.",
    wijken: [
      "Centrum",
      "Noord",
      "Zuid",
      "West",
      "Oost",
      "Kralingen",
      "Feijenoord",
      "Charlois",
      "IJsselmonde",
      "Prins Alexander",
    ],
    postcodes: "3000-3099",
    introText:
      "Rotterdam is de tweede stad van Nederland met diverse wijken. Wij bieden professionele woningontruiming in heel Rotterdam tegen de laagste prijs, gegarandeerd.",
    waaromKiezen: [
      "Uitgebreide kennis van alle Rotterdamse wijken",
      "24/7 bereikbaar voor spoedgevallen",
      "Ervaring met hoogbouw en monumentale panden",
      "Laagste prijs garantie",
      "Groot team voor grote projecten",
    ],
    situaties: [
      {
        title: "Hoogbouw appartementen Rotterdam",
        description:
          "Rotterdam kent veel hoogbouw. Wij hebben ruime ervaring met ontruiming van appartementen in hoge gebouwen.",
      },
      {
        title: "Ontruiming Rotterdamse wijken",
        description: "Van Noord tot Zuid, van Centrum tot Prins Alexander - wij werken overal.",
      },
      {
        title: "Spoedontruiming Rotterdam",
        description: "Bij spoed zijn wij vaak binnen enkele uren beschikbaar in Rotterdam.",
      },
      {
        title: "Bedrijfsontruiming Rotterdam",
        description: "Ook voor kantoren en bedrijfspanden in Rotterdam.",
      },
    ],
    faq: [
      {
        question: "Werken jullie in alle Rotterdamse wijken?",
        answer:
          "Ja, wij werken in heel Rotterdam: Centrum, Noord, Zuid, West, Oost, Kralingen, Feijenoord, Charlois, IJsselmonde en Prins Alexander.",
      },
      {
        question: "Zijn jullie 24/7 bereikbaar?",
        answer: "Voor spoedgevallen zijn wij 24/7 bereikbaar in Rotterdam.",
      },
      {
        question: "Hebben jullie ervaring met hoogbouw?",
        answer:
          "Ja, wij hebben uitgebreide ervaring met hoogbouw en gebruiken de juiste apparatuur en bescherming.",
      },
    ],
  },
  schiedam: {
    slug: "schiedam",
    naam: "Schiedam",
    regio: "Rijnmond",
    title: "Woningontruiming Schiedam - Laagste Prijs Garantie",
    description:
      "Professionele woningontruiming in Schiedam. Vindt u goedkoper? Wij duiken onder die prijs! Alle wijken, snel bereikbaar.",
    wijken: ["Schiedam Centrum", "Nieuwland", "Groenoord", "Kethel", "Spaland"],
    postcodes: "3110-3125",
    introText:
      "Schiedam grenst aan Rotterdam en kent diverse wijken. Wij bieden professionele woningontruiming in heel Schiedam tegen de laagste prijs, gegarandeerd.",
    waaromKiezen: [
      "Kennis van alle Schiedamse wijken",
      "Snel bereikbaar vanuit Rotterdam",
      "Ervaring met hoogbouw en oudere panden",
      "Laagste prijs garantie",
      "Flexibele planning",
    ],
    situaties: [
      {
        title: "Ontruiming historisch centrum",
        description: "Schiedam heeft een historisch centrum. Wij werken met respect voor oude panden.",
      },
      {
        title: "Hoogbouw Nieuwland",
        description: "Ervaring met hoogbouw in wijken als Nieuwland.",
      },
      {
        title: "Huurwoning opleveren",
        description: "Bezemschoon volgens alle voorwaarden.",
      },
      {
        title: "Spoedontruiming Schiedam",
        description: "Snel beschikbaar bij spoedgevallen.",
      },
    ],
    faq: [
      {
        question: "Werken jullie in alle wijken van Schiedam?",
        answer: "Ja, wij werken in heel Schiedam: Centrum, Nieuwland, Groenoord, Kethel en Spaland.",
      },
      {
        question: "Hoe snel kunnen jullie starten?",
        answer: "Meestal binnen 1-2 werkdagen, bij spoed binnen enkele uren.",
      },
      {
        question: "Hebben jullie ervaring met monumentale panden?",
        answer: "Ja, wij werken regelmatig in het historische centrum van Schiedam.",
      },
    ],
  },
  vlaardingen: {
    slug: "vlaardingen",
    naam: "Vlaardingen",
    regio: "Rijnmond",
    title: "Woningontruiming Vlaardingen - Laagste Prijs Garantie",
    description:
      "Professionele woningontruiming in Vlaardingen. Vindt u goedkoper? Wij duiken onder die prijs! Alle wijken bereikbaar.",
    wijken: ["Vlaardingen Centrum", "Westwijk", "Babberspolder", "Holy", "Ambacht"],
    postcodes: "3130-3137",
    introText:
      "Vlaardingen is een stad aan de Nieuwe Maas met diverse wijken. Wij bieden professionele woningontruiming in heel Vlaardingen tegen de laagste prijs, gegarandeerd.",
    waaromKiezen: [
      "Kennis van alle Vlaardingse wijken",
      "Goede bereikbaarheid",
      "Ervaring met verschillende woningtypes",
      "Laagste prijs garantie",
      "Persoonlijke service",
    ],
    situaties: [
      {
        title: "Woningontruiming alle wijken",
        description: "Van Centrum tot Westwijk, van Holy tot Ambacht.",
      },
      {
        title: "Seniorenverhuizing",
        description: "Begeleiding bij verhuizing naar zorginstelling.",
      },
      {
        title: "Huurwoning opleveren",
        description: "Bezemschoon volgens voorwaarden verhuurder.",
      },
      {
        title: "Spoedontruiming",
        description: "Snel beschikbaar bij spoed.",
      },
    ],
    faq: [
      {
        question: "Werken jullie in alle wijken?",
        answer: "Ja, wij werken in heel Vlaardingen inclusief alle wijken.",
      },
      {
        question: "Hoe snel kunnen jullie starten?",
        answer: "Meestal binnen 1-3 werkdagen, bij spoed sneller.",
      },
      {
        question: "Wat is de laagste prijs garantie?",
        answer: "Vindt u het elders goedkoper? Wij duiken onder die prijs!",
      },
    ],
  },
  "voorne-aan-zee": {
    slug: "voorne-aan-zee",
    naam: "Voorne aan Zee",
    regio: "Rijnmond",
    title: "Woningontruiming Voorne aan Zee - Laagste Prijs Garantie",
    description:
      "Professionele woningontruiming op Voorne aan Zee. Vindt u goedkoper? Wij duiken onder die prijs! Brielle, Hellevoetsluis en alle dorpen.",
    wijken: ["Brielle", "Hellevoetsluis", "Rockanje", "Tinte", "Oostvoorne", "Vierpolders", "Zwartewaal"],
    postcodes: "3230-3249",
    introText:
      "Voorne aan Zee bestaat uit 2 steden (Brielle en Hellevoetsluis) en 5 dorpen (Rockanje, Tinte, Oostvoorne, Vierpolders en Zwartewaal). Wij bieden professionele woningontruiming in alle kernen tegen de laagste prijs, gegarandeerd.",
    waaromKiezen: [
      "Actief in heel Voorne aan Zee - steden én dorpen",
      "Kennis van kustgebied en alle 7 kernen",
      "Ervaring met vakantiewoningen en permanente bewoning",
      "Laagste prijs garantie",
      "Flexibele planning voor zowel steden als dorpen",
    ],
    situaties: [
      {
        title: "Woningontruiming Brielle en Hellevoetsluis",
        description: "Professionele ontruiming in beide steden van Voorne aan Zee.",
      },
      {
        title: "Ontruiming dorpen",
        description: "Ook in Rockanje, Tinte, Oostvoorne, Vierpolders en Zwartewaal zijn wij actief.",
      },
      {
        title: "Vakantiehuis ontruimen",
        description: "Veel vakantiewoningen in de kustkernen. Wij helpen bij ontruiming en bezemschoon opleveren.",
      },
      {
        title: "Ontruiming na overlijden",
        description: "Met respect en zorg helpen wij tijdens moeilijke periode.",
      },
    ],
    faq: [
      {
        question: "Werken jullie in alle kernen van Voorne aan Zee?",
        answer: "Ja, wij werken overal in Voorne aan Zee: Brielle, Hellevoetsluis, Rockanje, Tinte, Oostvoorne, Vierpolders en Zwartewaal.",
      },
      {
        question: "Ontruimen jullie ook vakantiewoningen?",
        answer: "Ja, wij hebben ruime ervaring met zowel vakantiewoningen als permanente bewoning in het kustgebied.",
      },
      {
        question: "Hoe snel kunnen jullie starten?",
        answer: "Meestal binnen 1-3 werkdagen, bij spoed binnen 24 uur.",
      },
    ],
  },
}

